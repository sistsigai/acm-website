...
export const DashboardUpcomingEvent: React.FC<DashboardUpcomingEventProps> = ({
  ...
  return (
    <div className="glass-panel rounded-4 p-4 h-100 d-flex flex-column">
      ...
      <div className="border border-secondary border-opacity-25 rounded-3 p-3 flex-grow-1 d-flex flex-column justify-content-between">
        <div>
          <h6 className="text-white mb-2 fs-5 fw-bold">{latestEvent.name}</h6>

          <div className="mb-3">
            <span className="admin-badge admin-badge-success">
              {latestEvent.totalRegistrations ?? 0} Registered
            </span>
          </div>
        ...
        {latestEvent.contactPersons && latestEvent.contactPersons.length > 0 && (
          <div className="mt-3 pt-3 border-top border-secondary border-opacity-25">
            <h6 className="text-white mb-2 small fw-semibold">
            ...
            <div className="d-flex flex-column gap-2">
              {latestEvent.contactPersons.map((cp, idx) => (
                ...
                >
                  <span className="text-white fw-medium small">{cp.name}</span>
